...
export default async (
  ...
  try {
    ...
    await checkIfVoucherExists(`Chargeback; dispute for ${invoice.number}`);

    const res = await sevDeskClient.get(
      `/Voucher?descriptionLike=${invoice.number}`
    );
    ...
    const data: components["schemas"]["saveVoucher"] = {
      voucher: {
        ...
        supplierName: null,
        description: `Chargeback; dispute for ${invoice.number}`,
    ...
    console.error(`Could not create dispute voucher for ${transaction.source}`);
    console.log(err);
